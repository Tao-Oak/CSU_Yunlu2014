...
public class CSUBuilding {
	 ...
	 */
	public double BUILDING_VALUE = Double.MAX_VALUE;
	...
	public static final EnumSet<StandardEntityConstants.Fieryness> ESTIMATED_BURNING = 
			EnumSet.of(StandardEntityConstants.Fieryness.HEATING,
	...
	public void addNeighbourBuilding(CSUBuilding neighbour) {
		...
		this.radiationNeighbourBuildingsId.add(neighbour.getSelfBuilding().getID());
	...
	private void initWalls(AdvancedWorldModel world) {
		...
		for (int i = 2; i < apexList.length; i++) {
			...
			if (wall.validate()) {
				...
				Logger.warn("Ignoring odd wall at building " + selfBuilding.getID().getValue());
			}
			lastX = tempX;
			lastY = tempY;
		}
	...
	public void initWallValue(FireBrigadeWorld world) {
		...
		for (Enumeration<CSUBuilding> e = this.connectedBuildingTable.keys(); e.hasMoreElements();) {
	...
	/** Determines whehther this building is burning. */
	...
	public List<EntityID> getAreasInExtinguishableRange() {
		if (areasInExtinguishableRange == null || areasInExtinguishableRange.isEmpty()) {
...
//			}
	 ...
	 * value decrease the initial energy value, all the water will be
	...
	public void initSimulatorValues() {
		...
		Logger.info("Initialised the simulator values for building " + selfBuilding.getID() 
				+ ": ground area = " + selfBuilding.getGroundArea() + ", floors = " + selfBuilding.getFloors() 
				+ ", volume = " + volume + ", initial fuel = " + initFuel + ", energy capacity = " + getCapacity());
	}
	...
	public double getEstimatedTemperature() {
		...
		if (Double.isNaN(rv)) {
			Logger.warn("Building " + selfBuilding.getID() + " getTemperature returned NaN");
	...
	public double getRadiationEnergy() {
		double t = this.getEstimatedTemperature() + 273; // Assume ambient temperature is 293 Kelvin.
		double radEn = (t * t * t * t) * RADIATION_COEFFICIENT * STEFAN_BOLTZMANN_CONSTANT * totalWallArea;
		
		if (selfBuilding.getID().getValue() == 23545) {
			Logger.debug("Getting radiation energy for building " + selfBuilding.getID().getValue());
			Logger.debug("t = " + t);
			...
			Logger.debug("Stefan-Boltzmann constant: " + STEFAN_BOLTZMANN_CONSTANT);
...
//					+ " setEnergy: " + value + " --- " + world.getAgent().getID() + " " + invokeMethod);
//		}
	 ...
	 * 
	 * @return true when this building is watered in last cycle. Otherwise, false.
	 */
...
/* ------------------------------------------- other operations ------------------------------------------ */
	public boolean isExtinguished() {
		return this.getEstimatedFieryness() >= 4 && this.getEstimatedFieryness() < 8;
	}
